...
{
    /// <summary>
    ///  林密将苏飞版的HttpHelper DotNet4.0改为2.0
    /// </summary>
    public class HttpHelper
    {
        ...
        /// Fiddler中Raw生成C#代码
        /// </summary>
        /// <param name="rawPath">Raw文件路径，默认：d:\raw.txt</param>
        /// <returns>HttpHelperCode</returns>
        public static string Raw2Code(string rawPath = "D:\\raw.txt")
        {
            try
            {
                var ran = new Random().Next(1111111, 9999999);
                var rawText = System.IO.File.ReadAllText(rawPath, System.Text.Encoding.Default);
                var newLin = new char[] { '\r', '\n' };
                string[] lins = rawText.Substring(0, 4).Equals("POST") ? rawText.Substring(0, rawText.IndexOf("\r\n\r\n")).Split(newLin) : rawText.Split(newLin);

                System.Text.StringBuilder sbResult = new System.Text.StringBuilder("");
                sbResult.AppendFormat("HttpHelper httpHelper = new HttpHelper();            \r\n            HttpItem httpItem{0} = new HttpItem()\r\n            {{", ran);
                System.Text.StringBuilder sbHeader = new System.Text.StringBuilder();
                var firstLin = lins[0].Split(new char[] { ' ' });
                sbResult.AppendFormat("\r\n                Method=\"{0}\",\r\n", firstLin[0]);
                sbResult.AppendFormat("\r\n                URL=\"{0}\",\r\n", firstLin[1]);
                for (int i = 1; i < lins.Length; i++)
                {
                    var item = lins[i];
                    var lin = System.Text.RegularExpressions.Regex.Split(item, ": ");
                    if (lin.Length == 2)
                    {
                        var linLeft = lin[0];
                        switch (linLeft)
                        {
                            case "Referer":
                                sbResult.AppendFormat("                Referer=\"{0}\",\r\n", lin[1]);
                                break;
                            case "Cookie":
                                sbResult.AppendFormat("                Cookie=\"{0}\",\r\n", lin[1]);
                                break;
                            case "Accept":
                                sbResult.AppendFormat("                Accept=\"{0}\",\r\n", lin[1]);
                                break;
                            case "User-Agent":
                                sbResult.AppendFormat("                UserAgent=\"{0}\",\r\n", lin[1]);
                                break;
                            case "ContentType":
                            case "Content-Type":
                                sbResult.AppendFormat("                ContentType=\"{0}\",\r\n", lin[1]);
                                break;
                            case "Host":
                            case "DNT":
                            case "Content-Length":
                            ...
                            case "Accep":
                            default:
                                sbHeader.AppendFormat("            httpItem{0}.Header.Add(\"{1}\",\"{2}\");\r\n", ran, lin[0], lin[1].Replace("\"", "\\\""));//引号替换为  \"
                                break;
                        }
                    }
                }
                if (firstLin[0].Equals("POST"))
                {
                    var postdata = rawText.Substring(rawText.IndexOf("\r\n\r\n") + 4);
                    sbResult.AppendFormat("                Postdata=@\"{0}\",\r\n", postdata.Replace("\"", "\"\""));//引号替换为双引号
                }
                sbResult.Append("            };\r\n");
                sbResult.Append(sbHeader);
                sbResult.AppendFormat("            var htmlReslut{0}=httpHelper.GetHtml(httpItem{0});", ran);
            ...
            //    //ClentCertificates = "",
            //    PostEncoding = System.Text.Encoding.Default,
        ...
        /// <param name="item">参数类对象</param>
        /// <returns>返回HttpResult类型</returns>
        public byte[] GetHtml2(HttpItem objhttpitem)
        {
            //返回参数
            HttpResult result = new HttpResult();
            try
        ...
        public static HttpResult GetHtml(string url, string cookie = "")
        {
            return new HttpHelper().GetHtml(new HttpItem() { URL = url, Cookie = cookie });
        }


        public static System.Drawing.Image GetImage(string url, string cookie = "")
        {
            var httpResult = new HttpHelper().GetHtml(new HttpItem() { URL = url, Cookie = cookie, ResultType = ResultType.Byte });


            return System.Drawing.Bitmap.FromStream(new MemoryStream(httpResult.ResultByte), true); ;
